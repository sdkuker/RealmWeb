    ...
    function ($, Logger, MovementManeuverCollection) {
        ...
        MovementManeuverWarehouse = function () {
            ...
            this.getMovementManeuversForDifficultyWithDefaultForAdd = function (aMovementManeuverDifficultyModelObject) {
                var deferred = $.Deferred();
                var cacheKey = null;
                var myDifficultyId = null;
                if (aMovementManeuverDifficultyModelObject && aMovementManeuverDifficultyModelObject.get('id')) {
                    cacheKey = movementManeuversForDifficultyBaseKey + '-' + aMovementManeuverDifficultyModelObject.get('id') + ':withDefault';
                    myDifficultyId = aMovementManeuverDifficultyModelObject.get('id')
                } else {
                    cacheKey = movementManeuversForDifficultyBaseKey + ':withDefault';
                }

                if (cache[cacheKey]) {
                    deferred.resolve(cache[cacheKey]);
                } else {
                    if (aMovementManeuverDifficultyModelObject) {
                        var defaultManeuver = {};
                        defaultManeuver['result'] = 'Add New Maneuver On This Row';
                        defaultManeuver['minimumRollValue'] = 0;
                        defaultManeuver['maximumRollValue'] = 0;
                        cache[cacheKey] = new MovementManeuverCollection(defaultManeuver, { difficultyId: myDifficultyId });
                    } else {
                        cache[cacheKey] = new MovementManeuverCollection(null, { difficultyId: myDifficultyId });
                    }
                    cache[cacheKey].on('sync', function (collection) {
                        deferred.resolve(collection);
                    })
